...
class TableOperationsTestCase(unittest.TestCase):
    ...
    def test_insert_row(self):
        row = ["Gary", 2, "boy"]
        position = 2
        self.table.rows.insert(position, row)
        self.assertEqual(len(self.table.rows), 6)
    ...
    def test_contains(self):
        self.assertTrue("rank" in self.table.columns)
        self.assertFalse("rankk" in self.table.columns)
...
| こんにちは |  2   |  boy   |
+------------+------+--------+"""
        self.table.rows.append([u"こんにちは", 2, "boy"])
        self.assertEqual(string, self.table.get_string())
